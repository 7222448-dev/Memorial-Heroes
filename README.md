...
**Крок 4 (рекомендовано). Підключити Volume для постійного зберігання даних.**

Без Volume дані БД та завантажені файли зникають при кожному redeploy. Це нормально для тестування, але для бойового використання потрібен Volume:

1. У вашому проекті на Railway натисніть `+ Create` (вгорі праворуч)
2. Виберіть `Volume`
3. Виберіть існуючий сервіс (memorial)
4. У полі `Mount Path` введіть: `/app/data`
5. Натисніть `Add`. Railway сам зробить redeploy

Що відбувається після цього: код автоматично визначає змінну `RAILWAY_VOLUME_MOUNT_PATH`, яку Railway виставляє при підключенні Volume, і починає писати БД та файли туди. У логах деплою побачите рядок:

```
💾 Volume підключено: /app/data (дані переживуть redeploy)
```

Усе нове, що ви додасте через адмінку — герої, фото, аудіо, відео — буде жити постійно. Безкоштовний план Railway надає ~5 ГБ; цього достатньо для тисяч записів з фотографіями.
